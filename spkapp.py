...
df_ranking = df_ranking[['Kode', 'Alternatif_y', 'Bobot Akhir']].rename(columns={'Alternatif_y': 'Nama Game'})
df_ranking = df_ranking.sort_values(by='Bobot Akhir', ascending=False).reset_index(drop=True)
...
st.sidebar.markdown("---")
st.sidebar.caption("© 2023 Kelompok 5 - TI Bilingual")
...
if page == "Metode GADA":
    ...
    with tab2:
        ...
        # Step 2: FIX applied here (subset formatting)
...
# -----------------------------------------------------------------------------
# HALAMAN 2: HASIL PERANGKINGAN & CREDIT
# -----------------------------------------------------------------------------
elif page == "Hasil Perangkingan":
    st.title("🏆 Hasil Perangkingan")

    # Membuat Tab untuk Ranking dan Credit
    tab_rank, tab_credit = st.tabs(["📊 Perangkingan & Grafik", "👥 Credit"])

    # --- TAB 1: HASIL PERANGKINGAN ---
    with tab_rank:
        st.subheader("Hasil Akhir & Rekomendasi")
        ...
        with col_rank1:
            ...
            # Highlight juara 1
            def highlight_top(s):
                return ['background-color: #d4edda' if s.name == 1 else '' for _ in s]

            st.dataframe(
                df_ranking.style.format({"Bobot Akhir": "{:.8f}"}).apply(highlight_top, axis=1),
        ...
        with col_rank2:
            ...
            fig = px.bar(
                df_ranking.sort_values(by="Bobot Akhir", ascending=True),
                x="Bobot Akhir",
                y="Nama Game",
                orientation='h',
                text_auto='.4f',
                title="Perbandingan Bobot Akhir Alternatif",
                color="Bobot Akhir",
                color_continuous_scale="Viridis"
            )
            fig.update_layout(showlegend=False, xaxis_title="Bobot", yaxis_title="")
